...
void mahony_compute_error(struct mahony_filter *filter, float measured[3], float error[3])
{
	...
	float estimated_g[3];
	static float world_g[3] = {0.0f, 0.0f, 1.0f}; /* world coordinate gravity orientation */
...
void mahony_init(struct mahony_filter *filter, float kp, float ki, float i_limit, float sample_freq)
{
...
void mahony_update(struct mahony_filter *filter, float gyro[3], float accel[3])
{
	...
	quat_normalize(&filter->q); /* normalize */
}
