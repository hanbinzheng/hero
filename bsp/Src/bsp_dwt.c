...
/**
 * @brief private function used to check if DWT CYCCNT register overflows and update CYCCNT_RountCount
 * @attention this function assumes the time interval between two calls does not exceed one overflow
 *
 * @todo Better solution: set up a separate task for DWT time update?
 *        However, the original intention of using dwt is to ensure timing is not affected by factors 
 *        such as interrupts/tasks, so this implementation still has its significance
 *
 */
