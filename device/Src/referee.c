        ...
        struct referee_info *referee) 
{
        ...
        while(total_len != 0)
        {
                ...
                uint16_t len_data = rx_buff[1] | (rx_buff[2] << 8);
                ...
                uint16_t crc16 = data[len_data] | (data[len_data + 1] << 8);

                /* Validate data CRC16 */
                if(crc16 != get_crc16_check_sum(rx_buff, len_data + 7)){
        ...
        switch (cmd_id) {
                ...
                rx_buff += (len_data + 9); /* Advance by: 5 header + 2 cmd_id + 2 CRC16 */
                total_len -= (len_data + 9);
        }
}
