...
struct debug_type debug; //仅用于Ozone中数据可视化
 ...
 */
static int verify_crc16_check_sum(uint8_t *p_msg, uint16_t len)
{
 ...
 * @note
 **/
// void VT_Data_Process(struct vt03_data *vt, uint8_t *buff)
// {
// 	//接收到遥控器数据则喂狗，防止程序重启
// 	// HAL_IWDG_Refresh(&hiwdg);
// 	//接收
// 	if (verify_crc16_check_sum(buff, VT_FRAME_LENGTH)) {
// 		memcpy(vt, buff, VT_FRAME_LENGTH);
// 	}else {
...
void usart7_data_interpret(uint8_t *rx_buff) {
	...
	if (verify_crc16_check_sum(rx_buff, USART7_RX_FRAME_LEN)) {
		memcpy(&vt03_data, rx_buff, USART7_RX_FRAME_LEN);
	} else {
		crc_debug++;
	}

	debug.ch0 = (vt03_data.rc.bit.ch_0 - 1024) / 660.0f;
	debug.ch1 = (vt03_data.rc.bit.ch_1 - 1024) / 660.0f;
	debug.ch2 = (vt03_data.rc.bit.ch_2 - 1024) / 660.0f;
	debug.ch3 = (vt03_data.rc.bit.ch_3 - 1024) / 660.0f;
	debug.mode_sw = vt03_data.rc.bit.mode_sw;
	debug.pause = vt03_data.rc.bit.pause;
	debug.fn_1 = vt03_data.rc.bit.fn_1;
	debug.fn_2 = vt03_data.rc.bit.fn_2;
	debug.wheel = (vt03_data.rc.bit.wheel - 1024) / 660.0f;
	debug.trigger = vt03_data.rc.bit.trigger;
}